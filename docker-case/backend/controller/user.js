...
router.post("/create-user", upload.single("file"), async (req, res, next) => {
  try {
    ...
    const activationUrl = `http://192.168.56.16:3000/activation/${activationToken}`;
...
console.log("--- DEBUG: Ссылка для активации ---");
console.log(activationUrl); 
console.log("----------------------------------");

res.status(201).json({
  success: true,
  message: "Почта отключена. Ссылка в логах сервера!",
  activationUrl, // Мы передадим ссылку прямо в ответе для удобства
});
  } catch (err) {
